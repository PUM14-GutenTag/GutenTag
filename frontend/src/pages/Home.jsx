...
function Home() {
  ...
  return (
    <div className="home-container">
      {projectsShow ? 
        <div>
            <ul>{projects.map(result => <li key={result}><Project created={result[0]} name={result[1]} 
            projectType={result[2]} selectedColor={setColorCounter()}></Project></li>)}</ul>
          <div>
          </div>
          <Button variant='success'  onClick={toggleProjects}>Create project</Button>
        </div>
        :
        <div>
          <Button variant='success' onClick={toggleProjects}>Back to home</Button>
